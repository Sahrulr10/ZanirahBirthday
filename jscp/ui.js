...
function spawnHeartPhotosCentered() {
    document.querySelectorAll('.photo').forEach(el => el.remove()); // hapus foto lama yang nyangkut
...
function checkBookFinished() {
    ...
    if (currentPage === lastPageIndex && lastPage && lastPage.classList.contains('flipped')) {
        if (!isBookFinished) {
            ...
            setTimeout(() => {
                const currentSettings = window.settings || settings;
                const finalVideoWrapper = document.getElementById('finalVideoWrapper');
                const finalVideo = document.getElementById('finalVideo');

                // Sembunyikan buku dulu, biar nggak numpuk sama video
                const book = document.getElementById('book');
                const bookContainer = document.querySelector('.book-container');
                const contentDisplay = document.getElementById('contentDisplay');
                ...
                if (finalVideoWrapper && finalVideo) {
                    showStars();
                    finalVideoWrapper.style.display = 'flex';
                    finalVideo.play();

                    finalVideo.addEventListener('ended', () => {
                        finalVideoWrapper.style.display = 'none';
                        if (currentSettings.enableHeart) {
                            startHeartEffect();
                        }
                    }, { once: true });
                } else if (currentSettings.enableHeart) {
                    startHeartEffect();
                }
            }, 1000);
        }
...
// Coba autoplay begitu halaman selesai load
window.addEventListener('load', () => {
    birthdayAudio.play().then(() => {
        ...
        // Diblokir browser, akan nyala otomatis begitu ada klik/tap pertama
    });
});

// Fallback: begitu ada interaksi PERTAMA di mana pun (bukan cuma di buku), musik langsung nyala
const forcePlayMusicAnywhere = () => {
...
document.addEventListener('touchstart', forcePlayMusicAnywhere, { passive: true, once: true });
document.addEventListener('mousedown', forcePlayMusicAnywhere, { once: true });
document.addEventListener('click', forcePlayMusicAnywhere, { once: true });